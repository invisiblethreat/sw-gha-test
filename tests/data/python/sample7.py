...
os.environ['COMPANY_VERSION_NUMBER'] = LATEST_VERSION_NUMBER
...
class Upgrader():
    ...
    def is_stable_staging(self):
        ...
        return 'COMPANY_BUILD_BRANCH' in app_env and (app_env['COMPANY_BUILD_BRANCH'] == 'staging' or app_env['COMPANY_BUILD_BRANCH'] == 'stable')
    ...
    def run(self, script, version=LATEST_VERSION_NUMBER):
        ...
        script_stdout = open('/var/log/company/' + script + '_' + version + '.log', 'ab')
        script_stderr = open('/var/log/company/' + script + '_' + version + '.err', 'ab')
