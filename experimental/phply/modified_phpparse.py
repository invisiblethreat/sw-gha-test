...
def p_top_statement_namespace(p):
    ...
    if len(p) == 4:
        p[0] = ast.Namespace(p[2], [], lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 5:
        p[0] = ast.Namespace(None, p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Namespace(p[2], p[4], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_top_statement_constant(p):
    'top_statement : CONST constant_declarations SEMI'
    p[0] = ast.ConstantDeclarations(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_top_statement_use(p):
    'top_statement : USE use_declarations SEMI'
    p[0] = ast.UseDeclarations(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_use_declaration(p):
    ...
    if len(p) == 2:
        p[0] = ast.UseDeclaration(p[1], None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 3:
        p[0] = ast.UseDeclaration(p[1] + p[2], None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 4:
        p[0] = ast.UseDeclaration(p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
    else:
        p[0] = ast.UseDeclaration(p[1] + p[2], p[4], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_constant_declaration(p):
    ...
    p[0] = ast.ConstantDeclaration(p[1], p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_inner_statement_yield(p):
    '''inner_statement : YIELD SEMI
                       | YIELD expr SEMI'''
    if len(p) == 3:
        p[0] = ast.Yield(None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Yield(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_block(p):
    'statement : LBRACE inner_statement_list RBRACE'
    p[0] = ast.Block(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_statement_if(p):
    ...
    if len(p) == 8:
        p[0] = ast.If(p[3], p[5], p[6], p[7], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.If(p[3], ast.Block(p[6], lineno=p.lineno(5), lexpos=p.lexpos(5)),
                      p[7], p[8], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_while(p):
    'statement : WHILE LPAREN expr RPAREN while_statement'
    p[0] = ast.While(p[3], p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_do_while(p):
    'statement : DO statement WHILE LPAREN expr RPAREN SEMI'
    p[0] = ast.DoWhile(p[2], p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_for(p):
    'statement : FOR LPAREN for_expr SEMI for_expr SEMI for_expr RPAREN for_statement'
    p[0] = ast.For(p[3], p[5], p[7], p[9], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_foreach(p):
    'statement : FOREACH LPAREN expr AS foreach_variable foreach_optional_arg RPAREN foreach_statement'
    if p[6] is None:
        p[0] = ast.Foreach(p[3], None, p[5], p[8], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Foreach(p[3], p[5].name, p[6], p[8], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_switch(p):
    'statement : SWITCH LPAREN expr RPAREN switch_case_list'
    p[0] = ast.Switch(p[3], p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_break(p):
    '''statement : BREAK SEMI
                 | BREAK expr SEMI'''
    if len(p) == 3:
        p[0] = ast.Break(None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Break(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_continue(p):
    '''statement : CONTINUE SEMI
                 | CONTINUE expr SEMI'''
    if len(p) == 3:
        p[0] = ast.Continue(None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Continue(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_return(p):
    '''statement : RETURN SEMI
                 | RETURN expr SEMI'''
    if len(p) == 3:
        p[0] = ast.Return(None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Return(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_global(p):
    'statement : GLOBAL global_var_list SEMI'
    p[0] = ast.Global(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_static(p):
    'statement : STATIC static_var_list SEMI'
    p[0] = ast.Static(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_echo(p):
    'statement : ECHO echo_expr_list SEMI'
    p[0] = ast.Echo(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_inline_html(p):
    'statement : INLINE_HTML'
    p[0] = ast.InlineHTML(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_statement_unset(p):
    ...
    p[0] = ast.Unset(p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_statement_try(p):
    ...
    p[0] = ast.Try(p[3], p[5], p[6], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_additional_catches(p):
    '''additional_catches : additional_catches CATCH LPAREN fully_qualified_class_name VARIABLE RPAREN LBRACE inner_statement_list RBRACE
                          | empty'''
    if len(p) == 10:
        p[0] = p[1] + [ast.Catch(p[4], ast.Variable(p[5], lineno=p.lineno(5), lexpos=p.lexpos(5)),
                                 p[8], lineno=p.lineno(2), lexpos=p.lexpos(2))]
    else:
        p[0] = []
...
def p_maybe_finally(p):
    '''maybe_finally : FINALLY LBRACE inner_statement_list RBRACE
                     | empty'''
    if len(p) == 5:
        p[0] = ast.Finally(p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
    else:
        p[0] = None

def p_statement_throw(p):
    'statement : THROW expr SEMI'
    p[0] = ast.Throw(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_statement_declare(p):
    'statement : DECLARE LPAREN declare_list RPAREN declare_statement'
    p[0] = ast.Declare(p[3], p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_declare_list(p):
    '''declare_list : STRING EQUALS static_scalar
                    | declare_list COMMA STRING EQUALS static_scalar'''
    if len(p) == 4:
        p[0] = [ast.Directive(p[1], p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))]
    else:
        p[0] = p[1] + [ast.Directive(p[3], p[5], lineno=p.lineno(2), lexpos=p.lexpos(2))]
...
def p_elseif_list(p):
    ...
    if len(p) == 2:
        ...
        p[0] = p[1] + [ast.ElseIf(p[4], p[6], lineno=p.lineno(2), lexpos=p.lexpos(2))]

def p_else_single(p):
    '''else_single : empty
                   | ELSE statement'''
    if len(p) == 3:
        p[0] = ast.Else(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_new_else_single(p):
    ...
    if len(p) == 4:
        p[0] = ast.Else(ast.Block(p[3], lineno=p.lineno(2), lexpos=p.lexpos(2)),
...
def p_foreach_variable(p):
    ...
    if len(p) == 5:
        p[0] = ast.ForeachVariable(p[3], False, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 2:
        p[0] = ast.ForeachVariable(p[1], False, lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.ForeachVariable(p[2], True, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_case_list(p):
    ...
    if len(p) == 6:
        p[0] = p[1] + [ast.Case(p[3], p[5], lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 5:
        p[0] = p[1] + [ast.Default(p[4], lineno=p.lineno(2), lexpos=p.lexpos(2))]
    else:
        p[0] = []
...
def p_global_var(p):
    ...
    if len(p) == 2:
        p[0] = ast.Variable(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 3:
        p[0] = ast.Variable(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Variable(p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_static_var(p):
    '''static_var : VARIABLE EQUALS static_scalar
                  | VARIABLE'''
    if len(p) == 4:
        p[0] = ast.StaticVariable(p[1], p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.StaticVariable(p[1], None, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_function_declaration_statement(p):
    ...
    p[0] = ast.Function(p[3], p[5], p[8], p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    ...
    if len(p) == 8:
        ...
        for s in p[6]:
            if isinstance(s, ast.TraitUse):
                ...
                stmts.append(s)
        p[0] = ast.Class(p[2], p[1], p[3], p[4], traits, stmts, lineno=p.lineno(2), lexpos=p.lexpos(2))
    elif len(p) == 7:
        p[0] = ast.Interface(p[2], p[3], p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        traits = []
        stmts = []
        for s in p[4]:
            if isinstance(s, ast.TraitUse):
                ...
                stmts.append(s)
        p[0] = ast.Trait(p[2], traits, stmts, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_trait_member(p):
    '''trait_member : fully_qualified_class_name DOUBLE_COLON STRING
                    | STRING'''
    if len(p) == 4:
        p[0] = ast.StaticProperty(p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
    else:
        p[0] = p[1]
...
def p_trait_statement(p):
    ...
    if len(p) == 9:
        p[0] = ast.Method(p[4], p[1], p[6], p[8], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
    elif len(p) == 6:
        p[0] = ast.TraitUse(p[2], p[4], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        if p[1] == 'use':
            p[0] = ast.TraitUse(p[2], [], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = ast.ClassVariables(p[1], p[2], lineno=p.lineno(3), lexpos=p.lexpos(3))
...
def p_class_statement(p):
    ...
    if len(p) == 9:
        ...
        p[0] = ast.ClassConstants(p[1], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_class_variable_declaration_initial(p):
    ...
    if len(p) == 6:
        p[0] = p[1] + [ast.ClassVariable(p[3], p[5], lineno=p.lineno(2), lexpos=p.lexpos(2))]
    else:
        p[0] = [ast.ClassVariable(p[1], p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))]

def p_class_variable_declaration_no_initial(p):
    '''class_variable_declaration : class_variable_declaration COMMA VARIABLE
                                  | VARIABLE'''
    if len(p) == 4:
        p[0] = p[1] + [ast.ClassVariable(p[3], None, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    else:
        p[0] = [ast.ClassVariable(p[1], None, lineno=p.lineno(1), lexpos=p.lexpos(1))]
...
def p_class_constant_declaration(p):
    ...
    if len(p) == 6:
        p[0] = p[1] + [ast.ClassConstant(p[3], p[5], lineno=p.lineno(2), lexpos=p.lexpos(2))]
    else:
        p[0] = [ast.ClassConstant(p[2], p[4], lineno=p.lineno(1), lexpos=p.lexpos(1))]
...
def p_parameter(p):
    ...
    if len(p) == 2: # VARIABLE
        p[0] = ast.FormalParameter(p[1], None, False, None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 3 and p[1] == '&': # AND VARIABLE
        p[0] = ast.FormalParameter(p[2], None, True, None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 3 and p[1] != '&': # STRING VARIABLE
        p[0] = ast.FormalParameter(p[2], None, False, p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 4 and p[2] != '&': # VARIABLE EQUALS static_scalar
        p[0] = ast.FormalParameter(p[1], p[3], False, None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 4 and p[2] == '&': # STRING AND VARIABLE
        p[0] = ast.FormalParameter(p[3], None, True, p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 5 and p[1] == '&': # AND VARIABLE EQUALS static_scalar
        p[0] = ast.FormalParameter(p[2], p[4], True, None, lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 5 and p[1] != '&': # class_name VARIABLE EQUALS static_scalar
        p[0] = ast.FormalParameter(p[2], p[4], False, p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else: # STRING AND VARIABLE EQUALS static_scalar
        p[0] = ast.FormalParameter(p[3], p[5], True, p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_variable(p):
    'expr : variable'
    p[0] = p[1]

def p_expr_assign(p: yacc.YaccProduction):
    '''expr : variable EQUALS expr
            | variable EQUALS AND expr'''
    if len(p) == 5:
        p[0] = ast.Assignment(p[1], p[4], True, lineno=p.lineno(2), lexpos=p.lexpos(2))
    else:
        p[0] = ast.Assignment(p[1], p[3], False, lineno=p.lineno(2), lexpos=p.lexpos(2))

def p_expr_new(p):
    'expr : NEW class_name_reference ctor_arguments'
    p[0] = ast.New(p[2], p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_expr_objectop(p):
    ...
    if params is not None:
        p[0] = ast.MethodCall(p[1], name, params, lineno=p.lineno(3), lexpos=p.lexpos(3))
    else:
        p[0] = ast.ObjectProperty(p[1], name, lineno=p.lineno(3), lexpos=p.lexpos(3))
...
def p_dynamic_class_name_reference(p):
    ...
    if len(p) == 5:
        name, dims = p[3]
        p[0] = ast.ObjectProperty(p[1], name, lineno=p.lineno(2), lexpos=p.lexpos(2))
        for class_, dim, lineno in dims:
            p[0] = class_(p[0], dim, lineno=lineno)
        for name, dims in p[4]:
            p[0] = ast.ObjectProperty(p[0], name, lineno=p.lineno(2), lexpos=p.lexpos(2))
            for class_, dim, lineno in dims:
                p[0] = class_(p[0], dim, lineno=lineno)
    else:
        p[0] = p[1]
...
def p_expr_clone(p):
    'expr : CLONE expr'
    p[0] = ast.Clone(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_list_assign(p):
    'expr : LIST LPAREN assignment_list RPAREN EQUALS expr'
    p[0] = ast.ListAssignment(p[3], p[6], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_variable(p):
    ...
    if len(p) == 6:
        ...
        params = p[4]
        if params is not None:
            p[0] = ast.MethodCall(p[1], name, params, lineno=p.lineno(2), lexpos=p.lexpos(2))
        else:
            p[0] = ast.ObjectProperty(p[1], name, lineno=p.lineno(2), lexpos=p.lexpos(2))
        for class_, dim, lineno in dims:
            p[0] = class_(p[0], dim, lineno=lineno)
        for (name, dims), params in p[5]:
            if params is not None:
                p[0] = ast.MethodCall(p[0], name, params, lineno=p.lineno(2), lexpos=p.lexpos(2))
            else:
                p[0] = ast.ObjectProperty(p[0], name, lineno=p.lineno(2), lexpos=p.lexpos(2))
            for class_, dim, lineno in dims:
                p[0] = class_(p[0], dim, lineno=lineno)
...
def p_function_call(p):
    ...
    if len(p) == 5:
        p[0] = ast.FunctionCall(p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
    elif len(p) == 6:
        p[0] = ast.FunctionCall(p[1] + p[2], p[4], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.FunctionCall(p[1] + p[2] + p[3], p[5], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_function_call_static(p):
    ...
    p[0] = ast.StaticMethodCall(p[1], p[3], p[5], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_function_call_static_dynamic_name(p):
    ...
    p[0] = ast.StaticMethodCall(p[1], p[4], p[7], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_function_call_backtick_shell_exec(p):
    ...
    p[0] = ast.FunctionCall('shell_exec', [ast.Parameter(p[2], False)], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_static_member(p):
    ...
    if len(p) == 4:
        ...
        p[0] = ast.StaticProperty(p[1], p[4], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_variable_array_offset(p):
    ...
    p[0] = ast.ArrayOffset(p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_reference_variable_string_offset(p):
    ...
    p[0] = ast.StringOffset(p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_expr_scalar(p):
    'expr : scalar'
    # p[0] = p[1]
    if isinstance(p[1], ast.Scalar):
        lineno = p[1].lineno
        lexpos = p[1].lexpos
    else:
        lineno = p.lineno(1)
        lexpos = p.lexpos(1)
    p[0] = ast.ExprScalar(p[1], lineno=lineno, lexpos=lexpos)
...
def p_expr_array(p):
    ...
    p[0] = ast.Array(contents, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_non_empty_array_pair_list_item(p):
    '''non_empty_array_pair_list : non_empty_array_pair_list COMMA AND variable
                                 ...
                                 | expr'''
    if len(p) == 5:
        p[0] = p[1] + [ast.ArrayElement(None, p[4], True, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 4:
        p[0] = p[1] + [ast.ArrayElement(None, p[3], False, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 3:
        p[0] = [ast.ArrayElement(None, p[2], True, lineno=p.lineno(1), lexpos=p.lexpos(1))]
    else:
        p[0] = [ast.ArrayElement(None, p[1], False, lineno=p.lineno(1), lexpos=p.lexpos(1))]
...
def p_non_empty_array_pair_list_pair(p):
    ...
    if len(p) == 7:
        p[0] = p[1] + [ast.ArrayElement(p[3], p[6], True, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 6:
        p[0] = p[1] + [ast.ArrayElement(p[3], p[5], False, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 5:
        p[0] = [ast.ArrayElement(p[1], p[4], True, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    else:
        p[0] = [ast.ArrayElement(p[1], p[3], False, lineno=p.lineno(2), lexpos=p.lexpos(2))]
...
def p_function_call_parameter(p):
    '''function_call_parameter : expr
                               | AND variable'''
    if len(p) == 2:
        p[0] = ast.Parameter(p[1], False, lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Parameter(p[2], True, lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_function(p):
    'expr : FUNCTION is_reference LPAREN parameter_list RPAREN lexical_vars LBRACE inner_statement_list RBRACE'
    p[0] = ast.Closure(p[4], p[6], p[8], p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_lexical_var_list(p):
    '''lexical_var_list : lexical_var_list COMMA AND VARIABLE
                        ...
                        | VARIABLE'''
    if len(p) == 5:
        p[0] = p[1] + [ast.LexicalVariable(p[4], True, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 4:
        p[0] = p[1] + [ast.LexicalVariable(p[3], False, lineno=p.lineno(2), lexpos=p.lexpos(2))]
    elif len(p) == 3:
        p[0] = [ast.LexicalVariable(p[2], True, lineno=p.lineno(1), lexpos=p.lexpos(1))]
    else:
        p[0] = [ast.LexicalVariable(p[1], False, lineno=p.lineno(1), lexpos=p.lexpos(1))]
...
def p_expr_assign_op(p):
    ...
    p[0] = ast.AssignOp(p[2], p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_expr_binary_op(p):
    ...
    p[0] = ast.BinaryOp(p[2].lower(), p[1], p[3], lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_expr_ternary_op(p):
    ...
    p[0] = ast.TernaryOp(p[1], p[3], p[5], lineno=p.lineno(2), lexpos=p.lexpos(2))

def p_expr_short_ternary_op(p):
    'expr : expr QUESTION COLON expr'
    p[0] = ast.TernaryOp(p[1], p[1], p[4], lineno=p.lineno(2), lexpos=p.lexpos(2))

def p_expr_pre_incdec(p):
    '''expr : INC variable
            | DEC variable'''
    p[0] = ast.PreIncDecOp(p[1], p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_post_incdec(p):
    '''expr : variable INC
            | variable DEC'''
    p[0] = ast.PostIncDecOp(p[2], p[1], lineno=p.lineno(2), lexpos=p.lexpos(2))

def p_expr_cast_int(p):
    'expr : INT_CAST expr'
    p[0] = ast.Cast('int', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_double(p):
    'expr : DOUBLE_CAST expr'
    p[0] = ast.Cast('double', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_string(p):
    'expr : STRING_CAST expr'
    p[0] = ast.Cast('string', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_array(p):
    'expr : ARRAY_CAST expr'
    p[0] = ast.Cast('array', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_object(p):
    'expr : OBJECT_CAST expr'
    p[0] = ast.Cast('object', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_bool(p):
    'expr : BOOL_CAST expr'
    p[0] = ast.Cast('bool', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_unset(p):
    'expr : UNSET_CAST expr'
    p[0] = ast.Cast('unset', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_cast_binary(p):
    'expr : BINARY_CAST expr'
    p[0] = ast.Cast('binary', p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_isset(p):
    'expr : ISSET LPAREN isset_variables RPAREN'
    p[0] = ast.IsSet(p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_expr_empty(p):
    ...
    p[0] = ast.Empty(p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_eval(p):
    'expr : EVAL LPAREN expr RPAREN'
    p[0] = ast.Eval(p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_include(p):
    'expr : INCLUDE expr'
    p[0] = ast.Include(p[2], False, lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_include_once(p):
    'expr : INCLUDE_ONCE expr'
    p[0] = ast.Include(p[2], True, lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_require(p):
    'expr : REQUIRE expr'
    p[0] = ast.Require(p[2], False, lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_require_once(p):
    'expr : REQUIRE_ONCE expr'
    p[0] = ast.Require(p[2], True, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_expr_exit(p):
    ...
    if len(p) == 5:
        p[0] = ast.Exit(p[3], p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Exit(None, p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_print(p):
    'expr : PRINT expr'
    p[0] = ast.Print(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_expr_silence(p):
    'expr : AT expr'
    p[0] = ast.Silence(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_scalar(p):
    ...
    if len(p) == 4:
        ...
        if isinstance(p[2], str):
            p[0] = ast.Scalar(p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = p[2]
    elif len(p) == 5:
        if p[1] == 'b':
            if isinstance(p[3], str):
                p[0] = ast.Scalar(p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
            else:
                p[0] = p[3]
            # p[0] = p[3]
    else:
        ...
        if isinstance(p[1], str):
            p[0] = ast.Scalar(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
        else:
            p[0] = p[1]
...
def p_scalar_namespace_name(p):
    '''scalar : namespace_name
              ...
              | NAMESPACE NS_SEPARATOR namespace_name'''
    if len(p) == 2:
        p[0] = ast.Constant(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
    elif len(p) == 3:
        p[0] = ast.Constant(p[1] + p[2], lineno=p.lineno(1), lexpos=p.lexpos(1))
    else:
        p[0] = ast.Constant(p[1] + p[2] + p[3], lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_common_scalar_string(p):
    ...
    if len(p) == 3:
        ...
        value = p[1][1:-1].replace("\\'", "'").replace('\\\\', '\\')
        p[0] = ast.Scalar(value, lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_common_scalar_magic_line(p):
    'common_scalar : LINE'
    p[0] = ast.MagicConstant(p[1].upper(), p.lineno(1), lineno=p.lineno(1), lexpos=p.lexpos(1))

def p_common_scalar_magic_file(p):
    'common_scalar : FILE'
    value = getattr(p.lexer, 'filename', None)
    p[0] = ast.MagicConstant(p[1].upper(), value, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_common_scalar_magic_class(p):
    ...
    p[0] = ast.MagicConstant(p[1].upper(), None, lineno=p.lineno(1), lexpos=p.lexpos(1))
...
def p_encaps_list(p):
    ...
    if len(p) == 3:
        if p[1] == '':
            ...
            if isinstance(p[1], str):
                left = ast.Scalar(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1))
            else:
                left = p[1]
            p[0] = ast.BinaryOp('.', left, p[2], lineno=p.lineno(2), lexpos=p.lexpos(2))
    else:
        p[0] = ''
    ...
    if p[1] == '':
        ...
        if isinstance(p[1], string_type):
            ...
            if isinstance(p2, str):
                right = ast.Scalar(p2, lineno=p.lineno(2), lexpos=p.lexpos(2))
            else:
                right = p2
            p[0] = ast.BinaryOp('.', p[1], right,
                                lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_encaps_var_array_offset(p):
    ...
    p[0] = ast.ArrayOffset(ast.Variable(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1)), p[3],
                           lineno=p.lineno(2), lexpos=p.lexpos(2))

def p_encaps_var_object_property(p):
    'encaps_var : VARIABLE OBJECT_OPERATOR STRING'
    p[0] = ast.ObjectProperty(ast.Variable(p[1], lineno=p.lineno(1), lexpos=p.lexpos(1)), p[3],
                              lineno=p.lineno(2), lexpos=p.lexpos(2))
...
def p_encaps_var_dollar_curly_array_offset(p):
    ...
    p[0] = ast.ArrayOffset(ast.Variable('$' + p[2], lineno=p.lineno(2), lexpos=p.lexpos(2)), p[4],
