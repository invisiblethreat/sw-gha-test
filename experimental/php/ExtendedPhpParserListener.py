...
class ExtendedPhpParserListener(ParseTreeListener):
    ...
    def seen_context(self, ctx):
        if isinstance(ctx, PhpParser.KeyedVariableContext):
            self.current_key = ctx
            self.current_operator = None
            self.current_string = None
        elif isinstance(ctx, PhpParser.AssignmentOperatorContext):
            if ctx.getText() in ("=", '.='):
                if self.current_key is not None:
                    self.current_operator = ctx
        elif isinstance(ctx, PhpParser.StringContext):
            if self.current_key is not None and self.current_operator is not None:
                self.current_string = ctx
        
        if self.current_string is not None:
            # Found a key value pair
            key = self.current_key.getText()
            values = [n.getText() for n in self.current_string.getChildren() if n.getText() not in ("'", '"')]
            value = " ".join(values)
    ...
    def enterAssignmentOperator(self, ctx: PhpParser.AssignmentOperatorContext):
        ...
        # value = ctx.getText()
        # print(f"assignmentOperation: {value}")
        # pass
    ...
    def enterString(self, ctx: PhpParser.StringContext):
        self.seen_context(ctx)
        # values = [n.getText() for n in ctx.getChildren() if n.getText() not in ('"', "'")]
        # value = " ".join(values)
